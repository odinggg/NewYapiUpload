...
public class BuildJsonForDubbo {
     ...
     * @param: [e]
     * @return: java.util.ArrayList<com.com.github.odinggg.newyapiupload.dto.YapiDubboDTO>
     * @author: Hansen
     * @date: 2019/2/19
     */
    ...
    public YapiDubboDTO actionPerformed(PsiClass selectedClass, PsiMethod psiMethodTarget, Project project, PsiFile psiFile) {
        ...
        if (psiMethodTarget != null) {
            ...
            for (PsiParameter psiParameter : psiParameters) {
                if (psiParameter.getType() instanceof PsiPrimitiveType) {//如果是基本类型
                    KV kvClass = KV.create();
                    kvClass.set(psiParameter.getType()
                            .getCanonicalText(), NormalTypes.normalTypes.get(psiParameter.getType()
                            .getPresentableText()));
                    list.add(kvClass);
                    ...
                    PsiClass psiClassChild = JavaPsiFacade.getInstance(project)
                            .findClass(psiParameter.getType().getCanonicalText(), GlobalSearchScope
                                    .allScope(project));
                    KV kvObject = getFields(psiClassChild, project);
                    String classNameChild = ((PsiJavaFileImpl) psiClassChild.getContext()).getPackageName() + "." + psiClassChild
                            .getName();
                    KV kvClass = KV.create();
        ...
        try {
            ...
            String packageName = "/" + ((PsiJavaFileImpl) psiFile).getPackageName() + "." + selectedClass.getName() + "/1.0/" + psiMethodTarget
                    .getName();
